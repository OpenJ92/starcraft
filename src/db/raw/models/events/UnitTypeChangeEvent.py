...
class UnitTypeChangeEvent(db.Model):
    ...
    def process_dependancies(cls, obj, replay):
        ...
        unit_type = (
            None
            if not obj.unit_type_name
            else UNIT_TYPE.select_from_str_id(obj.unit_type_name, replay)
        )

        return {"info": info, "unit": unit, "unit_type": unit_type}

    columns = {"frame", "second", "name", "unit_id_index", "unit_id_recycle", "unit_id"}
