...
class SelectionEvent(db.Model):
    ...
    def process_dependancies(cls, _unit, obj, replay):
        ...
        player = (
            Nont if not obj.player else PLAYER.select_from_object(obj.player, replay)
        )
        unit = None if not _unit else OBJECT.select_from_object(_unit, replay)

        return {"info": info, "player": player, "unit": unit}

    columns = {"pid", "frame", "second", "name", "is_local", "control_group"}
