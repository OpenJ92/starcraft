...
class UnitDiedEvent(db.Model):
    ...
    def process_dependancies(cls, obj, replay):
        ...
        killing_unit = (
            None
            if not obj.killing_unit
            else OBJECT.select_from_object(obj.killing_unit, replay)
        )
        killing_player = (
            None
            if not obj.killing_player
            else PLAYER.select_from_object(obj.killing_player, replay)
        )
