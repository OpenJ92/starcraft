...
class INFO(db.Model):
    ...
    get_control_group_events = db.relationship(
        "GetControlGroupEvent", back_populates="info"
    )
    set_control_group_events = db.relationship(
        "SetControlGroupEvent", back_populates="info"
    )
    ...
    target_point_command_events = db.relationship(
        "TargetPointCommandEvent", back_populates="info"
    )
    target_unit_command_events = db.relationship(
        "TargetUnitCommandEvent", back_populates="info"
    )
    upgrade_complete_events = db.relationship(
        "UpgradeCompleteEvent", back_populates="info"
    )
    ...
    unit_type_change_events = db.relationship(
        "UnitTypeChangeEvent", back_populates="info"
    )
    ...
    def process_object(cls, replay):
        return {key: value for key, value in vars(replay).items() if key in cls.columns}
