...
class OBJECT(db.Model):
    ...
    target_unit_command_events = db.relationship(
        "TargetUnitCommandEvent", back_populates="target"
    )
    ...
    unit_type_change_events = db.relationship(
        "UnitTypeChangeEvent", back_populates="unit"
    )
    ...
    def process(cls, replay):
        ...
        if conditions:
            for _, obj in replay.objects.items():
                ...
                objs.append(cls(**data, **derived_data, **depend_data))
    ...
    columns = {"id", "started_at", "finished_at", "died_at"}
