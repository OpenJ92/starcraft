...
class ABILITY(db.Model):
    ...
    basic_command_events = db.relationship(
        "BasicCommandEvent", back_populates="ability"
    )
    target_point_command_events = db.relationship(
        "TargetPointCommandEvent", back_populates="ability"
    )
    target_unit_command_events = db.relationship(
        "TargetUnitCommandEvent", back_populates="ability"
    )
    ...
    def process_dependancies(cls, obj, replay):
        ...
        return {"build_unit": UT, "__UNIT_TYPE__": None if UT is None else UT.__id__}
