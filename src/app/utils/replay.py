...
def construct_table_row(info):
    ...
    players = [
        html.Td(
            children=[
                *[
                    html.Div(html.A(player.name, href=f"player/{player.id}"))
    ...
    return html.Tr(
        children=[
            ...
            *players,
            html.Td(children=[html.A("view", href=f"replay/{info.__id__}")]),
        ]
    )
...
def construct_table_body(query_info):
    return html.Tbody(children=[construct_table_row(info) for info in query_info])


def construct_replay_table():
    query_info = db.session.query(INFO).all()
    return html.Table(
        children=[construct_table_head(), construct_table_body(query_info)],
    )
